...
onready var Falling = get_node("/root/Game/Falling")
onready var Water = get_node("/root/Game/Water")
onready var Fire = get_node("/root/Game/Fire")
...
func _physics_process(delta):
	...
	for body in bodies:
		if body.is_in_group("Tiles"):
			...
			body.find_node("Break").emitting = true
			Falling.play()
			add_color(1.0)
			body.kill()
		add_trauma(2.0)
		if body.name == "Paddle":
			Water.play()
			body.find_node("Bubbles").emitting = true
		if body.name == "Wall":
			Fire.play()
